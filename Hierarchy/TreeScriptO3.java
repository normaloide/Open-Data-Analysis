// Questo script genera un file json contenente l'intero dataset in forma gerarchica temporale, in cui le foglie di ciascun osservazione (giorno) contengono il valore dell'inquinante O3 per quel giorno, secondo il formato name-value dove name indica il giorno e value la quantità di inquinante in microgrammi per metro cubo.

import java.util.Scanner;
import java.io.File;

public class TreeScriptO3
	{
			...
			for (int yCount = 2008; signalMore; yCount++)
				...
				while (signalMoreM)
					...
					for (; signalMoreD; currentRow++)			// NB: FORMATO DATA: DD/MM/AAAA // GIORNO: substring(0,2) // MESE: substring (3,5) // ANNO : substring (6,10)
						...
						if (tokenArr[4].equals("")) continue;		// Salta questa voce e passa immediatamente alla successiva se il dato è vuoto
						
						System.out.print ("{\"name\": \"" + tokenArr[0].substring(0,2) + "\", \"value\": " + tokenArr[4]);	// Altrimenti inseriscilo come valore del giorno
